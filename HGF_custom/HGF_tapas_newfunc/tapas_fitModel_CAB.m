...
function Sigma = safe_laplace_cov(H, ridge0)
    if nargin < 2; ridge0 = 1e-6; end
    ridge = ridge0;
    maxIter = 10;
    for k = 1:maxIter
        [R,p] = chol(H + ridge*eye(size(H)));
        if p == 0                 % success: PD achieved
            Sigma = inv(R)'*inv(R);   % (H+ridge I)^-1 via Cholesky
            return
        end
        ridge = ridge * 10;       % increase ridge and try again
    end
    error('Hessian could not be regularised after %d attempts', maxIter);
