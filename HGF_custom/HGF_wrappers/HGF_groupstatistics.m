function [stats] = HGF_groupstatistics(T,Vec,S)
...
ignorefields = {'dir','file','subj_pdat_idx','processed','subjects','groups','sessions','blocks','conds'};
...
for stf = 1:length(Stfields)
    ...
    for c = 1:size(C,1)
        ...
        gname = ['grps_' grps{C(c,1)} '_' grps{C(c,2)}];
        [stats.(gname).ranksum.p.(Stfields{stf}),~,stst] = ranksum(gdat{1},gdat{2},'method','approximate');
        stats.(gname).ranksum.z.(Stfields{stf}) = stst.zval;
        stats.(gname).signtest.(Stfields{stf})= signtest([St(:).(Stfields{stf})]);
    end
