...
%                 rand_choices(u1i) = D_sim(d).HGF(rep).sim.y(u1i(randperm(length(u1i))),1);
            ...
            fitted_choices_std(d,:) = std(fitted_choices_rep,0,1);
        ...
        varargout = {means_fitted,stds_fitted,means_randcorr_fitted,stds_randcorr_fitted,actual_condmeans,fitted_condmeans,actual_blockstimmeans,fitted_blockstimmeans,actual_macorrect,fitted_macorrect,actual_choices_all,fitted_choices_mean,fitted_choices_std};
    ...
    %                 ii = ~isnan(actual_rt) & ~isnan(fitted_rt);
            ...
            fitted_rt_std = nanstd(fitted_rt_all,0,2);
    ...
    %                 ii = ~isnan(fitted_choices_mean) & ~isnan(actual_rt) & ~isnan(fitted_rt_mean);
            ...
            fitted_rt_std_allsub(d,:) = fitted_rt_std';
        end
        if ~exist("brr_mean","var") brr_mean=struct; end
        varargout = {cc_mean,brr_mean,actual_condmeans,fitted_condmeans,actual_blockstimmeans,fitted_blockstimmeans,actual_rt_allsub,fitted_rt_allsub,fitted_rt_std_allsub};
