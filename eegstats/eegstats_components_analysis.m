...
if isfield(S,'select_freq') && ~isempty(S.select_freq)
    freq = find(ismember(S.freq_bands, S.select_freq));
    cumPnts = cumsum(S.freq_pnts);
    if freq == 1
        startIndex = 1;
    else
    ...
    temp = reshape(grpdata,[],D.prep.dim(1),D.prep.dim(2));
    grpdata = reshape(temp(:,:,indices),size(temp,1),[]);
    newdim(2) = length(indices);
end
...
%                     newdim(obs_dim)=1;
    ...
    for o = 1:length(subdata)
        ...
        if length(S.PCAmethod)>1 && any(strcmp(S.PCAmethod{2},'CCA'))
            ...
            % Define the desired number of workers
            c = parcluster('local');
            ...
            % Re-open the parallel pool with the specified number of workers
            myPool = parpool('local', numWorkers);
...
%                 minR = minR(minR<1);
            ...
            elseif S.cca_select_method == 2
                ...
                try
                    ...
                    nCCA = less(1)-1;
                catch % if there are none less
                    more = find(testmatrP > mean(testmatrandrP,2) + 2*std(testmatrandrP,[],2));
                    nCCA = more(end);
                end
                % also only keep if over a corr threshold
                more = find(testmatrP > S.min_corr);
                nCCA = min(nCCA,more(end));
            ...
            [O(o).W,O(o).mdata, ~, O(o).Wn, O(o).mdatan] = mccas(O(o).scores,nPCA,reg,minR(minYi),O(o).W,S);
...
%             end
        ...
        if length(S.PCAmethod)>1 && any(strcmp(S.PCAmethod{2},'CCA'))
            CCAs{u} = [];
            for o = 1:length(O)
                CCAs{u} = [CCAs{u}, O(o).PCAs{u}* (O(o).W(:,:,u)*O(o).Wn(1,u)) ]; % unscale the weights first
            end
            data_out=CCAs{u};
        else
            data_out=PCAs{u};
...
% weight maps
gavg = mean(oldgrpdata,1); % grand average of original EEG data
for o = 1:length(O)
    for cc = 1:NUM_FAC(2)
        for u=1:length(U)
...
for cc = 1:NUM_FAC(2)
    dat=squeeze(CCAw_gavg_signed(cc,:,:));
    ...
    % plot
    H=figure('name',['component ' num2str(cc) ', grand avg of subjects CCA/PCA weights']); 
    ...
    savefig(H,fullfile(S.savepth,['CCA' num2str(cc) '_grandavg_weights.fig']))
end

if size(origgrpdata,2)~=size(oldgrpdata,2)
    plot_old=1;
else
    plot_old=0;
end
...
for plotN=1:2
    ...
    % data is time-freq, a freq may have been selected (oldgrpdata) so plot
    % that one on it's own too
    if plotN==1
        data = origgrpdata;
        dim = D.prep.dim;
        titlelabel = 'all freq';
    elseif plotN==2 && plot_old
        data = oldgrpdata;
        dim = currdim;
        titlelabel = 'selected freq';
    else 
        continue
    end
    clear chantimecorrsub chantimecorr chantimecorravg


    % final chan-time data
    gavg = mean(data,1); % grand average of original EEG data
    for o = 1:length(O)
        for cc = 1:NUM_FAC(2)
            temp = [];
            for u=1:length(U)
                temp(1,:,u) = corr(plot_mdata{o}(cc,repidx{u},u)',data(iU==u,:),'type','Spearman'); % 3x60x20 x trials(57)xComp(1083=57x19)
            end
            chantimecorrsub(cc,:,:,:,o) = reshape(temp,dim(1),dim(2),[]); % each subject
    ...
    for cc = 1:NUM_FAC(2)
        ...
        % plot
        figure('name',['component ' num2str(cc) ', ' titlelabel ' , corr between CCA and EEG trials for each chan/time']); 
        ax(1) = subplot(nc+1,2,2); hold on
        plot(squeeze(max(topo,[],[1 2])),'b')
        plot([1,size(topo,3)],[thresh(2),thresh(2)],'b--')
        plot(squeeze(min(topo,[],[1 2])),'r')
    ...
    for cc = 1:NUM_FAC(2)
        ...
        % plot
        figure('name',['component ' num2str(cc) ', ' titlelabel ' , spatial and temporal variance maxima']); 
        pi=0;
        % s_gfp
        pi=pi+1;
        subplot(nc+1,2,pi); hold on
...
switch PCAmethod
    ...
    case 'PCA'
       % Define chunk size
        chunkSize = S.para; % You can adjust this based on available memory
        
        % Number of chunks
        numChunks = ceil(length(cdata) / chunkSize);
        
        % Initialize cell arrays to store results
        COEFF = cell(length(cdata), 1);
        score = cell(length(cdata), 1);
        COEFFrand = cell(length(cdata), 1);
        scorerand = cell(length(cdata), 1);
        explained_by_scree = zeros(length(cdata), 1);
        mfac = zeros(length(cdata), 1);
        nfac = zeros(length(cdata), 1);

        % Process each chunk separately
        for chunkIdx = 1:numChunks
            % Determine the range for this chunk
            startIdx = (chunkIdx - 1) * chunkSize + 1;
            endIdx = min(chunkIdx * chunkSize, length(cdata));
            ...
            parfor i = startIdx:endIdx
                ...
                if S.select_ncomp.scree
                    rand_cdata = zeros(size(cdata{i}));
                    % Randomly permute the data over trials
                    for col = 1:size(cdata{i}, 2)
                        rand_cdata(:, col) = cdata{i}(randperm(size(cdata{i}, 1)), col);
                    end
                    [COEFFrand{i}, scorerand{i},~,~,explainedrand] = pca(rand_cdata, 'NumComponents', NUM_FAC(1), 'Centered', false, 'Algorithm', 'eig');
