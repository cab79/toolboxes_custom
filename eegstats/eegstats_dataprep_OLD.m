...
% PCA on EEG is designed to work with grouped data, so if 'subject' output
% format is select, group it here and separate it out later
if strcmp(S.prep.output.format,'subject') && S.prep.calc.eeg.pca.on
    output_format = 'group';
else
    output_format = S.prep.output.format;
end
    ...
    if S.prep.calc.pred.zscore
        ...
        %else
            ...
            if S.prep.calc.eeg.pca.on
                ...
                catvar_names=dtab(:,ct).Properties.VariableNames;
...
%     end
    ...
    if ~isempty(S.prep.calc.eeg.cov_correct) 
        [U,~,iU]=unique(D.prep.dtab.ID,'stable');
        cov_names = S.prep.calc.eeg.cov_correct;
        formula = 'eeg ~ 1+';
        for cn = 1:length(cov_names)
            formula = [formula cov_names{cn} '+'];
        end
        formula = formula(1:length(formula)-1);
        
        for u=1:length(U)
            subtab = D.prep.dtab(iU==u,:);
            disp(['covariate correction, subject' num2str(u)]);
            
            % regression - obtain residuals
            for s = 1:size(D.prep.grpdata{1},2)
                subtab.eeg = D.prep.grpdata{1}(iU==u,s);
                lme = fitlme(subtab,formula);
                D.prep.grpdata{1}(iU==u,s) = residuals(lme);
            end
        end
...
% PCA on EEG is designed to work with grouped data, so if 'subject' output
% format is select, separate it out here
if strcmp(S.prep.output.format,'subject') && S.prep.calc.eeg.pca.on
