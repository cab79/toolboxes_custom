...
%     end
    ...
    if S.prep.calc.eeg.cca.on
        ...
        S.PCAmethod = 'CCA';
        S.CCA = 1;
        S.type = S.prep.calc.eeg.cca.type;
        S.type_obs_sep = S.prep.calc.eeg.cca.type_sep; % analyse "observation" separately. E.g. if temporal PCA, apply separately to each spatial channel?
        S.type_numfac = S.prep.calc.eeg.cca.numfac; % max number of factors
        S.data_in = S.prep.calc.eeg.cca.data_in; % type of data to analyse: trials or averaged conditions
        S.centre_outputs = S.prep.calc.eeg.cca.centre;
        S.standardise_outputs = S.prep.calc.eeg.cca.standardise;
        S.maxGig = S.prep.calc.eeg.cca.maxGig;
        S.normalise_cca_weights = S.prep.calc.eeg.cca.normalise_weights;
        Y=[];
        [D(d),grpdata]=eegstats_components_analysis(D(d),grpdata,S,Y);
%         
