function [position, RT, answer, out] = slideScale(screenPointer, question, rect, endPoints, varargin)
...
while answer == 0
    ...
    elseif strcmp(device, 'keyboard')
        ...
        newpress=0;
        if pressed
            thispress=firstPress(firstPress>0);
            if fpress ~= thispress
                newpress=1;
                fpress = thispress;
                pressedbutton = KbName(firstPress);
                out.pressbutton = [out.pressbutton pressedbutton];
                out.presstime = [out.presstime thispress];
            end
            KbQueueFlush; 
        end
        
        if strcmp(pressedbutton,'LeftArrow')
            x = x - length(scaleRange)/(100/incr);
        elseif strcmp(pressedbutton,'RightArrow')
            x = x + length(scaleRange)/(100/incr);
        end
    else
    ...
    DrawFormattedText(screenPointer, endPoints{1}, leftTick(1, 1) - textBounds(1, 3)/2,  rect(4)*(scalaPosition+0.1), [],[],[],[],[],[],[]); % Left point
    DrawFormattedText(screenPointer, endPoints{2}, rightTick(1, 1) - textBounds(2, 3)/2,  rect(4)*(scalaPosition+0.1), [],[],[],[],[],[],[]); % Right point
    ...
    if newpress
        out.rating = [out.rating round(position)];
        save(['scaleoutput_' sname],'out');
    end
    
    % Display position
    if displayPos
        DrawFormattedText(screenPointer, num2str(round(position)), 'center', rect(4)*(scalaPosition + 0.1)); 
    end
    ...
    elseif strcmp(device, 'keyboard')
        ...
        if strcmp(pressedbutton,responseKey)
            answer = 1;
        end
    else
        error('Unknown device');
