...
elseif strcmp(opt,'responded')
    ...
    if isfield(h.Settings.adaptive(atype),'signaltarget')
        sng = h.Seq.signal(stim,h.i);
        for i = 1:length(sng)
            signaltarget(i) = h.Settings.adaptive(atype).signaltarget{2}(h.Settings.adaptive(atype).signaltarget{1}==sng(i));
        end
    else
        signaltarget = h.Seq.signal(stim,h.i);
    end
    ...
    if isfield(h.Settings.adaptive,'response_type')
        if strcmp(h.Settings.adaptive(atype).response_type,'samediff') && length(correctsignal)>1
