...
switch opt
    ...
    % settings options
    h.SettingsOptions = {'Discrim_isochron_cues','Discrim_isochron','Discrim_random_cues','Discrim_random','Discrim_isochron_3freqcond','Discrim_isochron_2freqcond','Discrim_isochron_2freqcond_biasedhigh','Discrim_rand_reg'};
    
    case 'Discrim_isochron_2freqcond_biasedhigh'
    ...
    %h.Settings.ntrials = [30 30 10 10]; % biased_low
    h.Settings.ntrials = [10 10 30 30]; % biased_high
...
case 'Discrim_isochron_3freqcond'
...
%     4.	During trial: apply randomisation to standard and oddball to create “random” sequences.
    ...
    % Oddball method: intensity, index, pitch, channel
    h.Settings.PL.oddballmethod = 'duration_shuffle'; % can use same type for pattern only if oddball intensity is adaptive
    h.Settings.PL.oddballvalue = {[1 1],[1 2],[2 1],[2 2],[3 3],[3 4],[4 3],[4 4]}; % values to go into h.Seq.signal. One per oddprob row, or leave blank if determined from GUI
    h.Settings.PL.nonoddballstimvalue = {0 0 0 0 0 0 0 0};
    h.Settings.PL.oddballtype = 'classical'; % options: 'roving', 'classical'
    ...
    h.Settings.adaptive_general.seqtypecond = [1 1 1 1 2 2 2 2]; %if 'cond', associate each CP with an adaptive type
    h.Settings.adaptive_general.seqrandblocksize = 12; % should divide the number of trials in a set
    ...
    h.Settings.adaptive(1).slope_stimratio = 1; % number to divide stimulus level by, to calculate slope of ZEST. Should be smaller for thresholding, larger for adjustments during an expt
    %% ADAPTIVE 2
    h.Settings.adaptive(2)=h.Settings.adaptive(1);
