function h = MES_Seq_Discrim_PAL(h,opt)
...
switch opt
    ...
    % duration of trial in seconds
    h.Settings.trialdur = inf; % if 0, consecutive stimuli will occur with no gap
    h.Settings.nstim_trial = 3; % set to zero to be determined by stimdur. MARIE changed from 2 to 3. CAB: ok
    % Which stims that are targets for behavioural responses?
    h.Settings.target_stims = [1 2 3]; %MARIE changed from [1 2]. CAB: ok
    ...
    h.Settings.stim(1).stimrandind_durind = [1]; % select which values of signal to apply stimrandind to. MARIE changed from [1 2]. CAB: setting value doesn't appear to matter but needs to exist - value of 1 is fine.
    ...
    % duplicate
    for ns=[1 2 3] %MARIE changed from ns=[1 2] to ns=[1 2 3]. CAB: ok
    ...
    % within-trial frequency (s)
    h.Settings.wait=[sum(h.Settings.stim(1).dur{1})+1 sum(h.Settings.stim(2).dur{1})+1 0]; % one value per nstim
    ...
    h.Settings.PL.oddballvalue = {[1 2 2],[2 1 2],[2 2 1]}; % values to go into h.Seq.signal. One per oddprob row, or leave blank if determined from GUI. MARIE [1 1], [1 2], [2 1], [2 2]. CAB: I think this is ok - it means 1 out of the 3 will always be an odd-one-out and there will be no trials where all 3 are the same.
    h.Settings.PL.nonoddballstimvalue = {0 0 0}; %MARIE changed from {0 0 0 0}. CAB: ok
    h.Settings.PL.oddballtype = 'classical'; % options: 'roving', 'classical'
    ...
    h.Settings.PL.oddprob = [
        ...
        1/3 1/3 1/3
        1/3 1/3 1/3
        ]; %MARIE removed one line and changed to 1/3 instead of 1/2; CAB: ok, I think - I would need to check the sequence it generates
%     % index of row of oddprob that are standards and oddballs. Can be
%     % overridden by h.Settings.oddballvalue if using index
%     h.Settings.PL.standardind = 1; 
%     h.Settings.PL.oddind = 2; 
...
%     % randomise sets?
%     h.Settings.PL.rand_set = [0]; 
    h.Settings.ntrials = [10 10 10]; %MARIE changed from [10 10 10 10] % CAB: ok
    ...
    h.Settings.buttontype='key';
    % range of keyboard presses indicating a recordable response
    h.Settings.buttonopt = {'LeftArrow','DownArrow','RightArrow'}; %MARIE added the DownArrow option. CAB: ok
    % how early after start of trial can button press trigger the next trial? Empty if programmed
    % ISI
    ...
    h.Settings.adaptive(1).updown = [1 2 3];%MARIE changed from [1 2]. % CAB this will have no effect since "updown" is for a staircase procedure - instead we are using ZEST now
    % how many of each to run?
    h.Settings.adaptive(1).nRuns = 100*12;
    ...
    h.Settings.adaptive(1).trialsperrun = 1;
    % adaptive staircase: meanings of the buttonopt
    h.Settings.adaptive(1).buttonfun = {'LeftArrow','DownArrow','RightArrow'}; %MARIE added the DownArrow option. CAB: ok I think, but not sure what this does since I can't find the setting within the function!
    % adaptive staircase: corresponding signal values that would signify a
    % correct answer
    h.Settings.adaptive(1).signalval = [1 2 3];%MARIE changed from [1 2]. CAB: ok
    h.Settings.adaptive(1).signalcorrect = [1]; % correct response: CAB: the oddballvalue that is the correct response
    h.Settings.adaptive(1).signaltarget = {[1 2],[1 2]}; % translate actual signal values to signalval indices %MARIE what should this be? CAB: keep this as is.
    h.Settings.adaptive(1).response_type = '3AFC'; % options: 2AFC, 3AFC, samediff discrimination
    % reversals
    h.Settings.adaptive(1).reversals = [4;8;12];
    % stepsize
