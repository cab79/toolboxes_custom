...
function [EEG, cfg] = eeg_SASICA_cab(EEG,varargin)
% updated VEOG correlations code to allow more than 2 chahnels - "any"
% function
...
if cfg.autocorr.enable
    ...
    if ~noplot(1)
        ...
        for i = 1:numel(autocorr)
            ...
            cb = sprintf('eeg_SASICA_cab(EEG, ''pop_prop( %s, 0, %d, findobj(''''tag'''',''''comp%d''''), { ''''freqrange'''', [1 50] })'');', inputname(1), i, i);
...
if cfg.EOGcorr.enable
    ...
    if ~nocompute
        ...
        if numel(Veogchan) == 1 || numel(Veogchan) > 2
        ...
        if ~noV
            rejV=[];
            for ch = 1:size(VEOG,1)
                VEOGtemp = VEOG(ch,:,:);
                VEOGtemp = VEOGtemp(:);
                cV  = abs(corr(ICs,VEOGtemp))';
                corthreshV = readauto(corthreshV,cV,'+');
                rejV = [rejV; cV > corthreshV] ;
            end
            rejV = any(rejV,1);
        else
            cV = NaN(1,size(ICs,2));
...
if any(~noplot)
    if ~isempty([EEG.chanlocs.radius])% assume we have sensor locations...
        ...
        for ifig = 1:ceil((ncomp)/PLOTPERFIG)
            ...
            eeg_SASICA_cab(EEG,['pop_selectcomps(EEG, [' num2str(cmps) '],' num2str(ncomp) ');']);
            hfig(ifig) = gcf;
...
for i = 1:numel(buttonnums)
    ...
    mycb = regexprep(mycb,'pop_prop','eeg_SASICA_cab(EEG,''pop_prop');
    mycb = [mycb ''');'];
