...
%     subs = S.(S.func).designtab(:,find(strcmp(S.(S.func).designtab.Properties.VariableNames,'subjects')));
%     [unisubs,~,subsidx] = unique(subs,'stable');
%     for i = 1:length(S.(S.func).multoutlist.subjects)
%         RDsub(i) = mean(RD(ismember(S.(S.func).fileidx,find(subsidx==i))));
%     end
%     S.(S.func).multoutlist.RDsub = RDsub';%num2cell(sortRD');
    ...
    % each event type
    [unievents,~,eventidx] = unique(S.(S.func).eventidx,'stable');
    nsub = height(S.(S.func).multoutlist);
    for i = 1:length(unievents) 
        noemp = S.(S.func).noemp_vector(nsub*(i-1)+1:nsub*i);
        S.(S.func).multoutlist.(['RD_event' num2str(unievents(i))])(noemp) = RD(eventidx==i)';%num2cell(sortRD');
    end
