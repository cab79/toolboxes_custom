function EEG = manualRejectEEG(EEG)
% This function allows GUI-based manual rejection of epochs.

% TRIAL MARKING: Select any trials to be mark for rejection and click 'update marks'
...
while ~breakloop
    ...
    if  ~isempty(answer{6})         % remove trials (real)
        if strcmpi(answer{6},'MANREJ') % use marked trials from 'EEG' struct
            rej = manrejlist;
        else
    ...
    if  ~isempty(answer{3}) || ~isempty(answer{4}) && breakloop==0
        ...
        a = upper(allchan);
        b = upper(hfchans);
        indElec = find(ismember(a,b));
        
        % check if either say 'badchans' (this will overwrite previous indElec value)
