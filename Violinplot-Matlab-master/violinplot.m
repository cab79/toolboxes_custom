...
% This code is released under the terms of the BSD 3-clause license
    ...
    % CAB: allow specification of multiple colours
    iscolor = strcmp(varargin,'ViolinColor');
    colors_in = [];
    if any(iscolor) 
        col_ind = find(iscolor);
        if min(size(varargin{col_ind+1}))>1
    ...
    if isa(data, 'dataset') || isstruct(data) || istable(data)
        ...
        if hascategories && length(cats) == length(colnames)
            catnames=cats;
        end
