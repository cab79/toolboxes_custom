...
def bb_pipeline_struct(subject, runTopup, fileConfiguration):
    ...
    if (not 'T1' in fileConfiguration) or (fileConfiguration['T1'] == ''):
        ...
        if runTopup:
            #import subprocess
            #try:
            #    subprocess.check_output(["$BB_BIN_DIR/bb_structural_pipeline/bb_prepare_struct_fieldmap MNP_test1"], shell=True, stderr=subprocess.STDOUT)
            ...
            print(jobPREPAREFIELDMAP)
            print(jobMERGE)
            jobPREPAREFIELDMAP="0";
