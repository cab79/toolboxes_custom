...
def bb_pipeline_diff(subject, jobHold, fileConfiguration):
    ...
    jobPREBEDPOSTX =  LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 5   -N "bb_pre_bedpostx_cpu_'    + subject + '" -j ' + jobDTIFIT      + '  -l ' + logDir + ' $BB_BIN_DIR/bb_diffusion_pipeline/bb_bedpostx/bb_pre_bedpostx_cpu ' + baseDir + '/dMRI/dMRI')
    jobBEDPOSTX =     LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 190 -N "bb_bedpostx_cpu_'        + subject + '" -j ' + jobPREBEDPOSTX + '  -q $FSLGECUDAQ -l ' + logDir + ' $BB_BIN_DIR/bb_diffusion_pipeline/bb_bedpostx/bb_bedpostx_cpu ' + baseDir + '/dMRI/dMRI')
